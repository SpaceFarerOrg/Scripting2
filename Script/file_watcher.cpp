...
void CFileWatcher::OnFolderChange(const std::wstring& aDir)
{
	...
	for (WIN32_FIND_DATA& currentFile : currentFolderFiles)
	{
		...
		{
			std::string str(currentFile.cFileName);
			if (str.compare(currentFile.cFileName) == 0)
			{
				...
				{
					std::string thatString(currentFile.cFileName);
					std::wstring fileThatChangedPath = aDir + L"\\" + std::wstring(thatString.begin(), thatString.end());
...
bool CFileWatcher::WatchFileChangeWithDependencies(std::wstring aFile, callback_function_file aFunctionToCallOnChange)
{
	...
	while (getline(stream, line))
	{
		...
		if (found != std::string::npos)
		{
			...
			if (!foundFile.empty())
			{
				std::wstring wFoundFile(foundFile.begin(), foundFile.end());

				std::wstring depFile = directoryOfFile + L"\\" + wFoundFile;
...
std::vector<WIN32_FIND_DATA> CFileWatcher::GetAllFilesInFolder(std::wstring aDir)
{
	...
	std::string str(aDir.begin(), aDir.end());
